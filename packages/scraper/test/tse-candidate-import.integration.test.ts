...
describe('importTseCandidates', () => {
  ...
  it('keeps a same-name editorial profile separate until explicit identity review', async () => {
    ...
    expect(candidate).toEqual(expect.objectContaining({
      id: expect.not.stringMatching(/^editorial-maria$/),
      isOfficial: true,
    ...
    await expect(prisma.candidate.count()).resolves.toBe(2)
